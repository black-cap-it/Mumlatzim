...
public class AppUtilities {
    ...
    public static void faceBookDigitalLink(Activity activity) {
        try {
            ApplicationInfo applicationInfo = activity.getPackageManager().getApplicationInfo("com.facebook.katana", 0);
            if (applicationInfo.enabled) {
                updateLink(activity, "fb://facewebmodal/f?href=" + activity.getString(R.string.digital_url));
                return;
            }
        } catch (PackageManager.NameNotFoundException ignored) {
        }
        updateLink(activity, activity.getString(R.string.digital_url));
    }

    public static void faceBookfamilyLink(Activity activity) {
        try {
            ApplicationInfo applicationInfo = activity.getPackageManager().getApplicationInfo("com.facebook.katana", 0);
            if (applicationInfo.enabled) {
                updateLink(activity, "fb://facewebmodal/f?href=" + activity.getString(R.string.family_url));
                return;
            }
        } catch (PackageManager.NameNotFoundException ignored) {
        }
        updateLink(activity, activity.getString(R.string.family_url));
    }

    public static void faceBookgroupLink(Activity activity) {
        try {
            ApplicationInfo applicationInfo = activity.getPackageManager().getApplicationInfo("com.facebook.katana", 0);
            if (applicationInfo.enabled) {
                updateLink(activity, "fb://facewebmodal/f?href=" + activity.getString(R.string.group_url));
                return;
            }
        }
        catch (PackageManager.NameNotFoundException ignored)
        ...
        updateLink(activity, activity.getString(R.string.group_url));
    }

    public static void MessangerLink(Activity activity) {
        updateLink(activity, activity.getString(R.string.chat_url));
    }

    public static void telegramLink(Activity activity) {
        updateLink(activity, activity.getString(R.string.telegram_url));
    }
